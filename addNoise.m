...
for i = 1:numFrames
    Y_noise{1,i} = Y{1,numFrames} + 5*randn(frameHeight,frameWidth)
end

%adding noise to the U frame
for j = 1:numFrames
    U_noise{1,j} = U{1,numFrames} + 5*randn(UV_Height,UV_Width)
end

%adding noise to the V frame
for k = 1:numFrames
    V_noise{1,k} = V{1,numFrames} + 5*randn(UV_Height, UV_Width)
end
